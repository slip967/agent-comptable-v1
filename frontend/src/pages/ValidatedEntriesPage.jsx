...
function mergeEntries(localEntries, remoteEntries) { const byLine = new Map(); [...localEntries, ...remoteEntries].forEach((entry) => { const id = String(entry?.validation_id || entry?.validated_entry_id || entry?.line_id || entry?.id || (keyOf(entry) + "-" + (entry?.raw_text || entry?.description || "line"))); byLine.set(id, { ...byLine.get(id), ...entry }); }); return [...byLine.values()]; }

...
import { ChevronDown, CheckCircle2, Download, Eye, FileText, Pencil, Trash2, X, AlertTriangle, RotateCcw, Send, LoaderCircle } from "lucide-react";
import { useGlobalSearch } from "../context/SearchContext";
import { matchesGlobalSearch } from "../utils/search";
import { API_BASE_URL, fetchInvoicePdfPreview, fetchValidatedEntries, deleteHumanValidationItem, createHumanValidationItem, exportValidatedInvoiceToOdoo } from "../services/api";
...
const accountOf = (line) => String(line?.corrected_account || line?.human_validation_result?.corrected_account || line?.recommended_account || line?.accounting_account || line?.account || "").trim();
const accountLabelOf = (line) => value(line?.corrected_account_label || line?.human_validation_result?.corrected_account_label || line?.account_label || line?.recommended_account_label, "Compte comptable validé");
const accountsOf = (lines = []) => [...new Set(lines.map(accountOf).filter(Boolean))];
...
function odooMoveIdOf(entry, localExports = {}) {
  const invoiceId = keyOf(entry);
  const lines = Array.isArray(entry?.lines) ? entry.lines : [];
  return Number(
    localExports[invoiceId]
    || entry?.odoo_move_id
    || lines.find((line) => Number(line?.odoo_move_id || 0))?.odoo_move_id
    || 0,
  );
}
...
export default function ValidatedEntriesPage() {
  ...
  const exportToOdoo = async (invoice) => {
    const invoiceId = keyOf(invoice);
    if (!invoiceId) { setError("Identifiant de facture manquant pour l’export Odoo."); return; }
    setError("");
    setNotice("");
    setOdooLoadingId(invoiceId);
    try {
      const payload = await exportValidatedInvoiceToOdoo(invoiceId);
      const moveId = Number(payload?.move_id || 0);

...
SUMMARY_JSON = SCRIPT_DIR / "eleventh_wave_transport_btp_summary.json"
SUMMARY_MD = SCRIPT_DIR / "eleventh_wave_transport_btp_summary.md"
...
BTP_LABELS = (
    "MANCHE DE BALAI COCO PIN DOUILLE PLASTIQUE 120/24",
    "BALAI COCO 29 CM DOUILE RESINE",
    "BALAI COCO 29CM A VIS NOVIPRO",
    "BALANCE VRAC CODE A CLOU VIS BOULON KG",
    "DE RIVETEUSE UNIRIVET ALU",
    "MANCHE BALAI VIS ET ATTACHE PLASTIQ",
    "PLIAGE ALUMINIUM 10/10 ANODISE + PERFO (DETAIL 5) Long : 2000 Dev : 320 Eco contribution",
)
...
def build_items(helper, labels: tuple[str, ...], row_map: dict[str, dict], metier: str, selection_source: str) -> tuple[list[dict], list[str]]:
    ...
    for label in labels:
        ...
        suffix = f"Ajout manuel 11e vague depuis {row['source']}."
...
def main() -> int:
    ...
    transport_items, transport_missing = build_items(
        ...
        load_transport_rows(),
        "transport",
        "manual_eleventh_wave_transport_v1",
    )
    ...
    for file_name in TRANSPORT_TARGET_FILES:
        ...
        result = integrate_items(payload, transport_items, helper, "manual_eleventh_wave_transport_v1")
    ...
    for file_name in BTP_TARGET_FILES:
        path = SCRIPT_DIR / file_name
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
        result = integrate_items(payload, btp_items, helper, "manual_eleventh_wave_btp_v1")
        path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

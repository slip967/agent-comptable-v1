...
SUMMARY_JSON = SCRIPT_DIR / "twelfth_wave_transport_btp_summary.json"
SUMMARY_MD = SCRIPT_DIR / "twelfth_wave_transport_btp_summary.md"
...
TRANSPORT_LABELS = (
    "Bouclier avant complet remplacer",
    "Traverse avant remplacer (pare-chocs déposé)",
    "Filtre à air, remplacer",
    "Filtre à carburant, remplacer",
    "ESOE60ES B: remplacer le liquide de frein",
    "Supplément a la maintenance: faire l'appoint d'AdBlue (R)",
    "REMPLACEMENT DES PLAQUETTES DE FREIN AR",
    "Remplacer la batterie du reseau de bord (Apres controle)",
)
...
def build_items(helper, labels: tuple[str, ...], row_map: dict[str, dict], metier: str, selection_source: str) -> tuple[list[dict], list[str]]:
    ...
    for label in labels:
        ...
        suffix = f"Ajout manuel 12e vague depuis {row['source']}."
...
def main() -> int:
    ...
    transport_items, transport_missing = build_items(
        ...
        load_transport_rows(),
        "transport",
        "manual_twelfth_wave_transport_v1",
    )
    ...
    for file_name in TRANSPORT_TARGET_FILES:
        ...
        result = integrate_items(payload, transport_items, helper, "manual_twelfth_wave_transport_v1")
    ...
    for file_name in BTP_TARGET_FILES:
        path = SCRIPT_DIR / file_name
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
        result = integrate_items(payload, btp_items, helper, "manual_twelfth_wave_btp_v1")
        path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
